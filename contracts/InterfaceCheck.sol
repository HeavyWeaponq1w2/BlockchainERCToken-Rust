...
contract InterfaceCheck {
    function tokenfactory() external pure returns (bytes4) {
        return type(ITokenFactory).interfaceId;
    }

    function erc20WithData() external pure returns (bytes4) {
        return type(IERC20WithData).interfaceId;
    }

    function erc721WithData() external pure returns (bytes4) {
